from core.results_database import (show_system_info, show_results, show_problem_results, show_failed_tests, show_timeout_tests,
    show_status_summary, show_algorithm_summary, clear_results, count_results, show_dataset_results, show_process_diagnostic_events,
    show_process_diagnostics, show_process_diagnostic_events_compact, clear_algorithm_results, clear_process_diagnostics)


if __name__ == "__main__":
    # show_system_info()
    # show_results(limit=50)
    # show_dataset_results("random_int")
    # clear_results()
    # clear_algorithm_results("Sample Sort")
    show_algorithm_summary()
    show_timeout_tests()
    show_status_summary()
    show_failed_tests()
    show_problem_results()
    # count_results()

    # show_process_diagnostics()
    # show_process_diagnostic_events(5)
    # show_process_diagnostic_events_compact(14)
    # clear_process_diagnostics()
